...
async function safeFetch(
  ...
  if (!resolved)
    return { error: 'URL non autorisée (résolution DNS privée ou échec)', status: 400 };
...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    ...
    if (!result.ok) {
      return new Response(JSON.stringify({ error: `Erreur HTTP ${result.status}` }), {
        status: 502,
      });
    }
    ...
    if (items.length === 0) {
      return new Response(JSON.stringify({ error: 'Aucun article trouvé dans le flux' }), {
        status: 422,
      });
    }
    ...
    return new Response(JSON.stringify({ error: 'Erreur lors de la récupération du flux' }), {
      status: 500,
    });
  }
};
